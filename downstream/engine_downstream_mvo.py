...
#     # Concatenate all targets and predictions
    ...
    print(all_targets, flush = True)
    print(all_predictions, flush = True)
    tp = np.sum((1-all_targets)*(1-all_predictions))
    tn = np.sum(all_targets*all_predictions)
    fp = np.sum(all_targets*(1-all_predictions))
    fn = np.sum((1-all_targets)*(all_predictions))
    pos = np.sum(1-all_targets)
#     # Calculate confusion matrix
    indices_tp = np.where(((1-all_targets)*(1-all_predictions))==1)[0]
    indices_fn = np.where(((1-all_targets)*(all_predictions))==1)[0]
    ...
    # print(f"* {metric_str}")
    metric_fn.reset()
    return valid_stats, metrics, pos, tp, fp, tn, fn, indices_tp, indices_fn
