...
def main(config):
    ...
    dataset_ptbxl = ECGDataset(waves_train, labels_train)
    ...
    dataloader_with_labels_ptbxl = torch.utils.data.DataLoader(
        dataset_ptbxl, batch_size=128, shuffle=True, num_workers=2
    )
    with torch.no_grad():
    ...
    torch.save(
        all_features,
        "/home/nadja/ECG_JEPA_Git/embeddings/embeddings_ours"
        + config["pathology"]
        + ".pt",
    )
    torch.save(
        all_labels,
        "/home/nadja/ECG_JEPA_Git/embeddings/labels_ours" + config["pathology"] + ".pt",
    )

    print("waves mean: ", wave[0][0].mean())
    print(f"Representation shape: {repr.shape}")
    LAB = torch.zeros((all_features.shape[0], 7))
    LAB[: len(dataset_with_labels), :2] = all_labels[: len(dataset_with_labels)]
    LAB[len(dataset_with_labels) :, 2:] = all_labels_ptbxl[:]
    ...
    plt.savefig(
        "/home/nadja/ECG_JEPA_Git/embeddings/embedding_space_tsne_"
        + config["pathology"]
        + ".png"
    )
