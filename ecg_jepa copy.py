...
class Attention(nn.Module):
    ...
    def save_attn_map(self, attn):
        self.attn_map = attn
        attn.retain_grad()  # Ensure gradients are retained
        attn.register_hook(self.save_attn_grads)

    def save_attn_grads(self, grad):
    ...
    def forward(self, x, attention_mask=None):
        ...
        self.save_attn_map(attn)  # Store attention map
...
def compute_grad_cam(attn_module, normalize=True):
    """Computes Grad-CAM for the stored attention maps and gradients."""
    attn_map = attn_module.attn_map  # Shape: (B, num_heads, N, N)
    attn_grads = attn_module.attn_grads  # Shape: (B, num_heads, N, N)

    # Aggregate gradients across heads
    weights = attn_grads.mean(dim=1, keepdim=True)  # Shape: (B, 1, N, N)

    # Compute weighted attention map
    cam = (weights * attn_map).sum(dim=1)  # Shape: (B, N, N)
    cam = cam.mean(dim=1)  # Aggregate over tokens
