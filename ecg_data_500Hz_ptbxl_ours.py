...
class ECGDataset_pretrain(Dataset):
    ...
    def __getitem__(self, idx):
        wave = self.waves[idx]
        #print(f"Sample {idx} shape: {wave.shape}", flush = True)  # Debugging
...
def get_ecg_data(data_dir, reduced_lead=True, use_more=False, dx=False):
    ...
    if os.path.isdir(data_dir):
        ...
        if data_dir.endswith(".hea"):
            record_name = os.path.splitext(data_dir)[0]
...
class Code15Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        wave = transform(wave)
        #print('code shape, ', wave.shape, flush = True)
       ...
       # if len(os.listdir(os.path.join(self.data_dir, self.ecg_paths[idx]))) < 2:
        ...
        try:
            ...
            if wave is None:
                ...
                wave =  torch.zeros((1,8,5000))  # Replace with a default tensor
            wave = wave[0]
...
def waves_from_config(config, reduced_lead=True):
    ...
    return waves_train, waves_test, labels_train, labels_test
#/scratch/nadja/MIMICIV/files/p1112/p11125370/s48387619/48387619.hea